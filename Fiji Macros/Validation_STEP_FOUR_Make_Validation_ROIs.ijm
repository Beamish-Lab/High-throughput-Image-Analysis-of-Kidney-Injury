macro "Validation_STEP_FOUR_Make_Validation_ROIs [q]" {
	
//Macro helps the user extract validation rois.
//output are two new directories: 
//"ROI Locations" contains .roi files that show the location on the original images 
//"ROIs (Cropped Images-ALL)" contains cropped tif files from which some will be randomly selected for training
//these directories are placed in the the "[marker]-Extracted Validation Images (ALL)" same root directory as the open original image file directory.
...
//this macro is identical in function to Training_STEP_THREE_Make Training ROIs.ijm
...
output_root_directory = project_root_directory+marker+"-Extracted Validation Images (ALL)\\"; 
